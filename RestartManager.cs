...
namespace FivemStormManager
{
    static class RestartManager
    {
        ...
        private static TimeSpan next_restart  = new TimeSpan();
        private static bool skip_next_restart = false;
        private static bool emergency_restart = false;

        private static DbAccess logger = new DbAccess();

        public static void InitRestartManager()
        {
            SetServerTimes();
            SetProcess();
            StartServer();
        ...
        private static void CheckToBroadcastWarning()
        {
            ...
            if (warning_idx != -1 && !skip_next_restart)
            {
        ...
        private static void CheckToRestartServer()
        {
            ...
            if (now.Hours != next_restart.Hours && now.Minutes != next_restart.Minutes)
                return;

            if (!skip_next_restart)
                RestartServer();

            DetermineWhetherToSkipNextRestart();
            SetNextRestartTime(DateTime.Now.TimeOfDay);
        }

        private static void DetermineWhetherToSkipNextRestart()
        {
        ...
        private static void SetNextRestartTime(TimeSpan current_time)
        {

            int next_time_idx = times.FindIndex(obj => obj.Hours > current_time.Hours);
            if (next_time_idx != -1)
            {
                next_restart = times[next_time_idx];
                return;
            }

            if (current_time.Hours == times[times.Count - 1].Hours)
            {
                next_restart = times[0];
                return;
            }
        }
        ...
        public static void WriteToServer(string message, bool with_write_message = true)
        {
        ...
        public static void SkipNextRestart()
        {
            skip_next_restart = true;
        }

        public static void EmergencyRestart(TimeSpan emergency_restart_time)
        {
            next_restart      = emergency_restart_time;
